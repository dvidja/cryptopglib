...
namespace cryptopglib {
    ...
    public:
        ...
        std::vector<crypto::SignatureKeyInfo> signature_keys_info_;
    ...
    public:
        ...
        crypto::SignatureKeyInfo ReadSignatureMessage(const std::string &signature);

        crypto::SignatureResultInfo CheckSignature(const std::string &message, const std::string &public_key);

        crypto::SignatureResultInfo
        CheckSignature(const std::string &signature, const std::string &plain_text, const std::string &public_key);

        std::string SignMessage(const std::string &message, const std::string &key, const std::string &passhprase,
                                const int hash_algo, bool armored = false);
        ...
        DecryptMessage(const std::string &message, const std::string &secret_key, const std::string &passphrase);

        crypto::DecodedDataInfoPtr DecryptMessage(const std::string &message, std::vector<CharDataVector> attached_data,
                                          const std::string &secret_key, const std::string &passphrase);

        crypto::SignatureResultInfo CheckSignatureForDecryptedData(const CharDataVector &data, const std::string &signature,
                                                           const std::string &public_key);

        std::string EncryptData(const std::string &plain_text, const std::vector<std::string> &addressers_public_keys,
                                const std::string &own_public_key);

        KeyPairImpl GenerateKeyPair(const std::string &email, const std::string &passphrase);

        std::string EncryptAndSignMessage(const std::string &message, const std::vector<std::string> &encrypt_keys,
        ...
        crypto::CheckSignatureResult CheckKeySignature(const std::string &signed_key, const std::string &verification_key);

        std::string
        SignPublicKey(const std::string &public_key, const std::string &private_key, const std::string &passphrase);

        std::string
        ChangePassphrase(const std::string &private_key, const std::string &old_passwd, const std::string &new_passwd);
