...
#include "SymmetricallyEncryptedDataPacketParser.h"
...
PGPPacket* SymmetricallyEncryptedDataPacketParser::Parse(DataBuffer& data_buffer, bool partial, int c)
{
    ...
    if (partial)
    {
        SymmetricallyEncryptedDataPacket* packet = new SymmetricallyEncryptedDataPacket
        (mdc_ ? PT_SYMMETRIC_ENCRYTPED_AND_INTEGRITY_PROTECTED_DATA_PACKET : PT_SYMMETRICALLY_ENCRYPTED_DATA_PACKET);
